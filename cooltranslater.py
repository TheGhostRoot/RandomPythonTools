...
try:
    ...
    os.system("python -m pip install google-translate.py deep_translator")
    os.system("python3 -m pip install google-translate.py deep_translator")
    try:
        from google_translate_py import Translator
        import deep_translator as deep
    except ImportError:
    ...
    if translateToAllLangs:
        ...
        for key in languages.keys():
            ...
            for l in lines:
                if l != "":
                    ...
                    try:
                        t = translator.translate(word, "", key)
                    except Exception:
                        try:
                            translator2 = deep.GoogleTranslator(source="auto", target=key)
                            t = translator2.translate(text=word)
                        except Exception:
        ...
        for l in lines:
            if l != "":
                ...
                try:
                    t = translator.translate(word, "", langKey)
                except Exception:
                    try:
                        t = translator2.translate(text=word)
                    except Exception:
    ...
    if translateToAllLangs:
        for key in languages.keys():
            ...
            for l in lines:
                if l != "":
                    try:
                        t = translator.translate(l, "", key)
                    except Exception:
                        try:
                            translator2 = deep.GoogleTranslator(source="auto", target=key)
                            t = translator2.translate(text=l)
                        except Exception:
                            print(timeoutMessage)
                            exit()
                    if "„" in t:
        ...
        for l in lines:
            if l != "":
                try:
                    t = translator.translate(l, "", langKey)
                except Exception:
                    try:
                        t = translator2.translate(text=l)
                    except Exception:
